...
class Recognizer(object):
    ...
    def postprocess(self, boxes, inputs, thr):
        ...
        # } for i in indices]
# 检查输入数据一致性
        if not (len(class_ids) == len(boxes) == len(scores)):
            raise ValueError("Mismatch in lengths of class_ids, boxes, and scores.")

        # 确保 indices 的值在所有列表范围内
        valid_indices = [i for i in indices if 0 <= i < min(len(class_ids), len(boxes), len(scores))]
        ...
        for i in valid_indices:
            try:
                result.append({
                    "type": self.label_list[class_ids[i]].lower(),  # 确保 class_ids[i] 有效
                    "bbox": [float(t) for t in boxes[i].tolist()],  # 转换 boxes[i] 为列表并确保是数字
                    "score": float(scores[i])  # 确保 scores[i] 是数字
                })
            except IndexError as e:
                print(f"IndexError for index {i}: {e}")
                continue  # 跳过当前索引
        return result
