...
import openai
from bisheng.rag.nlp import is_chinese, is_english
from bisheng.rag.utils import num_tokens_from_string
...
LENGTH_NOTIFICATION_EN = "\n\n[Note: The output may be truncated due to length limitations]"
...
class LangChainCompletions:
    ...
    def create(self, model, messages, stream=False, **kwargs):
        ...
        chat_model = ChatOllama(
            model=model,
            base_url=self.parent_client.base_url,
            # 将其他参数转换为LangChain可接受的格式
            temperature=kwargs.get("temperature", 0.7),
            max_tokens=kwargs.get("max_tokens"),
            top_p=kwargs.get("top_p", 1.0),
            timeout=self.parent_client.timeout
        )
        ...
        if stream:
            return LangChainStreamResponse(chat_model, langchain_messages, kwargs)
        else:
            ...
            # 创建一个类似于OpenAI响应的对象
            return LangChainResponse(content, finish_reason, len(content.split()))

class LangChainChat:
    def __init__(self, parent_client):
        self.parent_client = parent_client
        self.completions = LangChainCompletions(parent_client)
...
class LangChainClient:
    def __init__(self, api_key, base_url, timeout=600):
        ...
        self.chat = LangChainChat(self)

class LangChainResponse:
    def __init__(self, content, finish_reason, total_tokens):
        self.choices = [LangChainChoice(content, finish_reason)]
        self.usage = LangChainUsage(total_tokens)

class LangChainChoice:
    def __init__(self, content, finish_reason):
        self.message = LangChainMessage(content)
...
class LangChainStreamResponse:
    def __init__(self, chat_model, messages, kwargs):
    ...
    def __iter__(self):
        ...
        try:
            ...
            # 构造错误响应
            error_chunk = LangChainStreamChunk("", "error", self.token_count)
            error_chunk.choices[0].delta.content = f"ERROR: {str(e)}"
            yield error_chunk

class LangChainStreamChunk:
    def __init__(self, content, finish_reason, total_tokens):
        self.choices = [LangChainStreamChoice(content, finish_reason)]
        self.usage = LangChainUsage(total_tokens)

class LangChainStreamChoice:
    def __init__(self, content, finish_reason):
        self.delta = LangChainDelta(content)
        self.finish_reason = finish_reason
...
class Base(ABC):
    def __init__(self, key, model_name, base_url):
        ...
        # 替换为我们的LangChain客户端
        self.client = LangChainClient(api_key=key, base_url=base_url, timeout=timeout)
