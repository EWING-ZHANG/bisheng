...
class Dealer:
    ...
    def search(self, req, idx_names: str | list[str], kb_ids: list[str], emb_mdl=None, highlight = False):
        ...
        if not qst:
            ...
            #如果有提供查询问题（qst），则执行基于问题的搜索，首先使用 self.qryr.question(qst) 生成问题的查询表达式。
            #如果没有提供嵌入模型（emb_mdl），则进行普通的文本搜索。
            #如果提供了嵌入模型，则通过 self.get_vector(qst, emb_mdl, topk, req.get("similarity", 0.1)) 生成问题的嵌入向量，并在搜索时结合文本匹配和向量匹配进行融合。融合方式是加权和（weighted_sum），权重为 0.05 和 0.95。
            if emb_mdl is None:
